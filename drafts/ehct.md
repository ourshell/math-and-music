...
where $$\Delta_{\text{EHCT}}(i)$$ is the cents difference for interval $$i$$ under the EHCT scale, and $$w_i$$ is a weight reflecting the **importance** of that interval (e.g., $$w_{3/2}$$ might be higher than $$w_{7/4}$$ if we care more about perfect fifths than harmonic sevenths).
...
where $$x_n$$ is the “scale degree” measured in **12-ET semitones**. In a **pure 12-ET system**, we have $$x_n = n$$ for $$n=0,1,\dots,11$$. In EHCT, the $$x_n$$ are **not** integers but real-valued, chosen to keep the entire scale as close to a 12-tone division as possible while reducing the interval errors for the prime ratios.
...
Although 7-limit intervals (e.g., $$7/4$$, $$7/6$$) rarely appear in standard Western harmony, they **frequently** appear in folk traditions, blues bends, and extended chords (e.g., 13th chords can approximate 7:4 overtones). Incorporating 7 in EHCT can yield a scale that also resonates with these expansions.
...
After enough iteration, we converge on a set $$\{x_0, x_1, \ldots, x_{11}\}$$ that yields **very low total error** for 3:2, 5:4, and 7:4.  
...
*(Feel free to adapt, expand, or remove references as suits your project.)*
